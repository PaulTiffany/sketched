...
const META_KEY='z0.inference.model-meta.v1';
...
function saveSelection(){if(!selected)return;const model=models.find(item=>item.id===selected);if(model)localStorage.setItem(META_KEY,JSON.stringify({provider:'openrouter',model:model.id,context_length:model.context||0,text_in:model.text,text_out:model.text,vision:model.vision,structured:model.structured,variable:model.variable,observed_at:new Date().toISOString()}));const hiddenModel=$('#modelId');if(hiddenModel)hiddenModel.value=selected;$('#openrouterChoice')?.click();setTimeout(()=>$('#useConnector')?.click(),0)}
