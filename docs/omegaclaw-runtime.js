...
window.fetch=(input,init)=>{
  ...
  if(cfg.mode==='omegaclaw'&&url.startsWith('https://openrouter.ai/api/v1/models')){
    return Promise.resolve(new Response(JSON.stringify({data:[{id:'runtime/omegaclaw',name:'REAL OMEGACLAW',pricing:{prompt:'0',completion:'0',request:'0'},architecture:{input_modalities:['text']},supported_parameters:['response_format'],context_length:32000}]}),{status:200,headers:{'Content-Type':'application/json'}}));
  }
